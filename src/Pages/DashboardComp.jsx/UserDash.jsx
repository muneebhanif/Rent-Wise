...
export default function UserDash() {
  ...
  const shown = agreements.filter(item => (filter === 'all' || (filter === 'history' ? ['completed', 'rejected'].includes(item.rentalState) : item.rentalState === filter)) && (item.listingId?.title || '').toLowerCase().includes(search.trim().toLowerCase()));
  return <Box>
    ...
    <Stats items={[{ label: 'Currently renting', value: count('rented'), description: 'Your active rentals', icon: CalendarDays, accent: true }, { label: 'Upcoming', value: count('upcoming'), description: 'Confirmed and starting soon', icon: Clock3 }, { label: 'Awaiting agreement', value: count('pending'), description: 'Not rented yet', icon: FileText }, { label: 'Completed', value: count('completed'), description: 'Your rental history', icon: CheckCircle2 }]} />
